...
import {
  getAxialOrthancStudies,
  getCoronalOrthancStudies,
  getSagittalOrthancStudies,
} from "../../../redux/studies/studies.action";
import { studiesSelector } from "../../../redux/studies/studies.selector";
import { RootState } from "../../../redux/store";
import {
  ArrowLeftRight, Camera, ChevronDown, Circle, Contrast,
  LayoutGrid, Move, Pencil, PenTool, RotateCcw, RotateCw,
  Ruler, Square, ZoomIn, ZoomOut,
} from "lucide-react";
import {
  ActiveTool, defaultPanel, LayoutMode, MeasurementItem,
  MeasurementMode, NormalizedPoint, PanelState, SinglePanelProps,
  StudyItem, TomographyProps, ViewMode, ViewCache,
} from "../VisorTypes/Visor.interface";
...
function SinglePanel({
  ...
  const currentIndexRef = useRef(panel.currentIndex);
  ...
  const [annotModal, setAnnotModal] = useState<{ point: NormalizedPoint } | null>(null);
  const [annotText,  setAnnotText]  = useState("");
  const annotInputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (annotModal && annotInputRef.current) {
      annotInputRef.current.focus();
    }
  }, [annotModal]);

  // W/L — compute from pixel luminance using fetch→blob→objectURL to avoid CORS taint
  const onPanelUpdateRef = useRef(onPanelUpdate);
  useEffect(() => { onPanelUpdateRef.current = onPanelUpdate; }, [onPanelUpdate]);

  useEffect(() => {
    const src = panel.images[panel.currentIndex];
    if (!src) { onPanelUpdateRef.current(p => ({ ...p, windowWidth: null, windowLevel: null })); return; }
    let cancelled = false;
    ...
    const compute = async () => {
      try {
        ...
        img.onload = () => {
          ...
          const W = Math.max(1, Math.min(128, img.naturalWidth  || 128));
          const H = Math.max(1, Math.min(128, img.naturalHeight || 128));
          ...
          try {
            ...
            if (!cancelled) {
              onPanelUpdateRef.current(p => ({
                ...p,
                windowWidth: Math.max(1, max - min),
                windowLevel: Math.round((max + min) / 2),
              }));
            }
          } catch { /* ignore */ }
        };
        ...
        img.src = objectUrl;
      } catch { /* network error or cancelled */ }
    };
  ...
  useEffect(() => {
    currentIndexRef.current = panel.currentIndex;
  }, [panel.currentIndex]);
  ...
  useEffect(() => {
    if (panel.cineActive && panel.images.length > 1) {
      ...
      cineRef.current = window.setInterval(() => {
        onPanelUpdate(p => ({ ...p, currentIndex: (p.currentIndex + 1) % p.images.length }));
      }, interval);
    } else {
      if (cineRef.current !== null) { window.clearInterval(cineRef.current); cineRef.current = null; }
    }
    return () => { if (cineRef.current !== null) { window.clearInterval(cineRef.current); cineRef.current = null; } };
  }, [panel.cineActive, panel.cineFps, panel.images.length]);

  const key          = `${panel.studyId}:${panel.view}:${panel.currentIndex}:${panelIndex}`;
  ...
  const getPoint = (cx: number, cy: number): NormalizedPoint | null => {
    ...
    if (!s || !dr.width || !dr.height) return null;
    const b = s.getBoundingClientRect();
    const ox = dr.x + panel.panOffset.x;
    const oy = dr.y + panel.panOffset.y;
    const lx = Math.max(ox, Math.min(ox + dr.width,  cx - b.left));
    const ly = Math.max(oy, Math.min(oy + dr.height, cy - b.top));
    return { x: (lx - ox) / dr.width, y: (ly - oy) / dr.height };
  };

  const toStage = (p: NormalizedPoint) => ({
    x: dr.x + p.x * dr.width  + panel.panOffset.x,
    y: dr.y + p.y * dr.height + panel.panOffset.y,
  });
  ...
  const freeA = (pts: NormalizedPoint[]) => {
    ...
    let a = 0;
    for (let i = 0; i < sc.length; i++) { const n = sc[(i + 1) % sc.length]; a += sc[i].x * n.y - n.x * sc[i].y; }
    return Math.abs(a / 2);
  };
  ...
  const buildLabel = (mode: MeasurementMode, pts: NormalizedPoint[]): string => {
    ...
    if (mode === "bidirectional") {
      ...
      const p1 = { x: cx - (nx / norm) * len, y: cy - (ny / norm) * len };
      const p2 = { x: cx + (nx / norm) * len, y: cy + (ny / norm) * len };
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (activeTool === "contrast" && isCtrRef.current) {
      const dx = e.clientX - ctrStart.current.x;
      const dy = e.clientY - ctrStart.current.y;
      onPanelUpdate(p => ({
        ...p,
  ...
  const onWheel = (e: React.WheelEvent) => {
    ...
    scrollTm.current = window.setTimeout(() => {
      ...
      if (delta > 0 && idx < panel.images.length - 1)
  ...
  useEffect(() => {
    ...
    const nativeWheel = (ev: WheelEvent) => {
      ...
      ev.stopPropagation();
      const idx = currentIndexRef.current;
      if (ev.deltaY > 0 && idx < panel.images.length - 1) {
        onPanelUpdate(p => ({ ...p, currentIndex: p.currentIndex + 1 }));
      } else if (ev.deltaY < 0 && idx > 0) {
        onPanelUpdate(p => ({ ...p, currentIndex: p.currentIndex - 1 }));
      }
    };
    stage.addEventListener("wheel", nativeWheel, { passive: false });
    return () => stage.removeEventListener("wheel", nativeWheel as EventListener);
  }, [onPanelUpdate, panel.images.length]);
  ...
  const renderM = (m: MeasurementItem) => {
    const dynLabel = m.mode === "annotation" ? (m.text || m.label) : buildLabel(m.mode, m.points);
    ...
    if (m.mode === "annotation") return (
      <g key={m.id}>
        ...
        {lbl(s.x + 20, s.y - 14, dynLabel, MEASURE_COLOR, m.id)}
    ...
    if (m.mode === "longitudinal" && e) return renderLine(m.id, s.x, s.y, e.x, e.y, dynLabel, MEASURE_COLOR);
    if (m.mode === "bidirectional" && e) {
      ...
      const p1 = { x: cx - (nx / norm) * len, y: cy - (ny / norm) * len };
      const p2 = { x: cx + (nx / norm) * len, y: cy + (ny / norm) * len };
      return renderLine(m.id, s.x, s.y, e.x, e.y, dynLabel, MEASURE_COLOR, false, { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y });
    }
    if (!e) return null;
    ...
    if (m.mode === "rectangle") return (
      <g key={m.id}>
        ...
        {[{ cx: x, cy: y }, { cx: x + w, cy: y }, { cx: x, cy: y + h }, { cx: x + w, cy: y + h }].map((p, i) =>
          <circle key={i} cx={p.cx} cy={p.cy} r={HANDLE_R} fill="transparent" stroke={MEASURE_COLOR} strokeWidth={1.5} />)}
        {lbl(x + w / 2, y - 8, dynLabel, MEASURE_COLOR, m.id)}
    ...
    if (m.mode === "ellipse") return (
      <g key={m.id}>
        ...
        {[{ cx: x + w / 2, cy: y }, { cx: x + w / 2, cy: y + h }, { cx: x, cy: y + h / 2 }, { cx: x + w, cy: y + h / 2 }].map((p, i) =>
          <circle key={i} cx={p.cx} cy={p.cy} r={HANDLE_R} fill="transparent" stroke={MEASURE_COLOR} strokeWidth={1.5} />)}
        {lbl(x + w / 2, y - 8, dynLabel, MEASURE_COLOR, m.id)}
    ...
    if (m.mode === "circle") {
      ...
      return (
        <g key={m.id}>
          ...
          {[{ cx, cy: cy - r }, { cx, cy: cy + r }, { cx: cx - r, cy }, { cx: cx + r, cy }].map((p, i) =>
            <circle key={i} cx={p.cx} cy={p.cy} r={HANDLE_R} fill="transparent" stroke={MEASURE_COLOR} strokeWidth={1.5} />)}
          {lbl(x + w / 2, y - 8, dynLabel, MEASURE_COLOR, m.id)}
    ...
    if (m.mode === "freehand") {
      ...
      return (
        <g key={m.id}>
          ...
          {lbl(c.x / spts.length + 6, c.y / spts.length - 6, dynLabel, MEASURE_COLOR, m.id)}
      ...
      {panel.loading ? (
        ...
        <div className="relative h-full w-full">
          <img
            ...
            style={{
              left: `${dr.x + panel.panOffset.x}px`,
              top: `${dr.y + panel.panOffset.y}px`,
              width: `${dr.width}px`,
              height: `${dr.height}px`,
          ...
          <div className="absolute bottom-2 left-2 z-10 pointer-events-none flex flex-col gap-0.5">
            <span className="text-[10px] font-mono text-slate-300">W:{panel.windowWidth ?? "—"} · L:{panel.windowLevel ?? "—"}</span>
            <span className="text-[10px] font-mono text-slate-400">{sp ? `${sp.x.toFixed(3)}×${sp.y.toFixed(3)} mm/px` : "sin calibración"}</span>
            <span className="text-[11px] font-mono font-semibold text-slate-100">{panel.currentIndex + 1}/{panel.images.length}</span>
          </div>
          ...
          {annotModal && (
            ...
            >
              <div
                className="w-72 rounded-2xl border border-slate-700 bg-[#0f172a] shadow-2xl p-5 flex flex-col gap-4"
                onPointerDown={(e) => e.stopPropagation()}
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center gap-2">
                  <div className="h-8 w-8 rounded-lg bg-yellow-500/15 flex items-center justify-center">
                    <Pencil className="h-4 w-4 text-yellow-400" strokeWidth={1.8} />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-white">Nueva anotación</p>
                    <p className="text-[10px] text-slate-400">Se anclará al punto seleccionado</p>
                  </div>
                </div>
                <input
                  ref={annotInputRef}
                  ...
                  onKeyDown={e => {
                    if (e.key === "Enter" && annotText.trim()) {
                      onCommit(panelIndex, { id: crypto.randomUUID(), mode: "annotation", points: [annotModal.point], text: annotText.trim(), label: annotText.trim() });
                      setAnnotModal(null); setAnnotText("");
                    }
                    if (e.key === "Escape") { setAnnotModal(null); setAnnotText(""); }
                  }}
                  placeholder="Escribe la anotación..."
                  className="w-full rounded-lg bg-slate-800 border border-slate-600 px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-yellow-500/60 focus:ring-1 focus:ring-yellow-500/30 transition-colors"
                />
                <div className="flex gap-2">
                  <button type="button" onClick={(e) => { e.stopPropagation(); setAnnotModal(null); setAnnotText(""); }}
                    className="flex-1 rounded-lg border border-slate-700 bg-transparent px-3 py-2 text-xs text-slate-300 hover:bg-slate-800 transition-colors">
                    Cancelar
                  </button>
                  <button type="button" disabled={!annotText.trim()}
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="flex-1 rounded-lg bg-yellow-500/20 border border-yellow-500/40 px-3 py-2 text-xs font-semibold text-yellow-300 hover:bg-yellow-500/30 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                    Agregar
                  </button>
                </div>
              </div>
...
export default function TomographyView({ tomography, studies, onBack }: TomographyProps) {
  ...
  const preloadAroundIndex = useCallback(async (urls: string[], currentIndex: number, radius = 12) => {
    if (!urls.length) return;
    const start = Math.max(0, currentIndex - radius);
    const end   = Math.min(urls.length - 1, currentIndex + radius);
    await Promise.all(urls.slice(start, end + 1).map(url => preloadImage(url)));
  }, [preloadImage]);

  const fetchForPanel = useCallback(async (pidx: number, studyId: string, view: ViewMode) => {
    const currentPanel = panelsRef.current[pidx];
    const cached = currentPanel?.viewCache?.[view];

    if (cached && cached.studyId === studyId && cached.images.length > 0) {
      setPanels(prev => {
        const next = [...prev];
        next[pidx] = {
          ...next[pidx],
          ...
          pixelSpacing: cached.pixelSpacing,
          panOffset:    cached.panOffset,
          brightness:   cached.brightness,
          contrast:     cached.contrast,
          rotation:     cached.rotation,
          zoom:         cached.zoom,
          inverted:     cached.inverted,
          cineActive:   cached.cineActive,
          cineFps:      cached.cineFps,
          loading:      false,
          loadingProgress: cached.images.length ? 100 : 0,
        };
        return next;
      });
      return;
    ...
    setPanels(prev => {
      ...
      next[pidx] = {
        ...next[pidx],
        loading: true, loadingProgress: 0,
        images: [], instanceIds: [], spacingByInstance: {},
        currentIndex: 0, pixelSpacing: null,
        panOffset: { x: 0, y: 0 },
        brightness: 256, contrast: 256,
        rotation: 0, zoom: 1, inverted: false, cineActive: false,
      };
      return next;
    });

    try {
      const study = view === "coronal"
        ? await dispatch(getCoronalOrthancStudies(studyId)).unwrap()
        : view === "sagital"
          ? await dispatch(getSagittalOrthancStudies(studyId)).unwrap()
          : await dispatch(getAxialOrthancStudies(studyId)).unwrap();

      const urls        = study.instances.map((i: { url: string }) => i.url);
      const instanceIds = study.instances.map((i: { instanceId: string }) => i.instanceId);
      const pixelSpacing = parseSpacing(study.pixelSpacing);

      if (urls[0]) await preloadImage(urls[0]);

      const cacheEntry: ViewCache = {
        studyId, images: urls, instanceIds, pixelSpacing,
        currentIndex: 0, panOffset: { x: 0, y: 0 },
        brightness: 256, contrast: 256,
        rotation: 0, zoom: 1, inverted: false, cineActive: false, cineFps: 10,
      };
      ...
      setPanels(prev => {
        ...
        next[pidx] = {
          ...next[pidx],
          studyId, view,
          images: urls, instanceIds, spacingByInstance: {},
          currentIndex: 0,
          loading: false, loadingProgress: urls[0] ? 5 : 0,
          pixelSpacing,
          viewCache: { ...next[pidx].viewCache, [view]: cacheEntry },
        };
        return next;
      });

      void preloadAroundIndex(urls, 0, 12).then(() => {
        setPanels(prev => {
          const next = [...prev];
          if (next[pidx].studyId === studyId) next[pidx] = { ...next[pidx], loadingProgress: 100 };
          return next;
        });
      });

    } catch (err) {
      console.error("fetchForPanel error:", err);
      setPanels(prev => {
        const next = [...prev];
        next[pidx] = { ...next[pidx], loading: false };
        return next;
      });
    }
  }, [dispatch, preloadImage, preloadAroundIndex]);
  ...
  useEffect(() => {
    ...
    fetchForPanel(0, studyId, "axial");
  }, [tomography.orthancStudyId, fetchForPanel]);

  const updatePanel = useCallback((idx: number) => (fn: (p: PanelState) => PanelState) => {
    setPanels(prev => {
      const next      = [...prev];
      const prevPanel = next[idx];
      const updated   = fn(prevPanel);

      if (updated.viewCache && updated.studyId && updated.view) {
        const cache = updated.viewCache[updated.view];
        if (cache && cache.studyId === updated.studyId) {
          updated.viewCache = {
            ...updated.viewCache,
            [updated.view]: {
              ...cache,
              ...
              zoom:         updated.zoom,
              inverted:     updated.inverted,
              cineActive:   updated.cineActive,
              cineFps:      updated.cineFps,
            },
          };
        }
      }
      ...
      if (updated.images.length > 0 && updated.currentIndex !== prevPanel.currentIndex) {
        void preloadAroundIndex(updated.images, updated.currentIndex, 12);
      }
  ...
  const handleCommit = useCallback((pidx: number, item: MeasurementItem) => {
    const p = panelsRef.current[pidx];
    const k = `${p.studyId}:${p.view}:${p.currentIndex}:${pidx}`;
    setMeasurementsByImage(prev => ({ ...prev, [k]: [...(prev[k] ?? []), item] }));
  }, []);

  const handleDelete = useCallback((pidx: number, id: string) => {
    const p = panelsRef.current[pidx];
    const k = `${p.studyId}:${p.view}:${p.currentIndex}:${pidx}`;
    setMeasurementsByImage(prev => ({ ...prev, [k]: (prev[k] ?? []).filter(m => m.id !== id) }));
  }, []);

  const activeView: ViewMode = panels[activePanel]?.view ?? "axial";

  const handleSelectView = useCallback((view: ViewMode) => {
    const studyId = panelsRef.current[activePanel]?.studyId || tomography.orthancStudyId || DEMO_STUDY_ID;
    fetchForPanel(activePanel, studyId, view);
    setIsViewMenuOpen(false);
  }, [activePanel, fetchForPanel, tomography.orthancStudyId]);

  const handleDrop = useCallback((pidx: number, studyId: string) => {
    const view = panelsRef.current[pidx]?.view ?? "axial";
    fetchForPanel(pidx, studyId, view);
  }, [fetchForPanel]);

  // ── Capture: draw image + SVG directly onto canvas — no screen sharing ─────
  const handleCapture = useCallback(async () => {
    const panel = panelsRef.current[activePanel];
    ...
    // dr = position of the image within the stage (includes panOffset via CSS)
    const dr = { x: ir.left - sr.left, y: ir.top - sr.top, width: ir.width, height: ir.height };
    const sc = 2;
    const canvas = document.createElement("canvas");
    canvas.width  = dr.width  * sc;
    canvas.height = dr.height * sc;
    const ctx = canvas.getContext("2d")!;
    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // Load image via fetch→blob to avoid CORS canvas taint
    try {
      const res = await fetch(panel.images[panel.currentIndex], { credentials: "include" });
      if (res.ok) {
        const blob = await res.blob();
        const blobUrl = URL.createObjectURL(blob);
        const src = new Image();
        await new Promise<void>(r => { src.onload = () => r(); src.onerror = () => r(); src.src = blobUrl; });
        // Apply rotation + zoom + filter, centered on the canvas
        ctx.save();
    ...
    } catch { /* CORS or network — canvas stays black, at least SVG will render */ }

    // Draw SVG overlay (measurements + annotations)
    if (svgEl) {
      const cl = svgEl.cloneNode(true) as SVGSVGElement;
      // viewBox maps the image-relative stage coords → canvas size
      cl.setAttribute("viewBox", `${dr.x} ${dr.y} ${dr.width} ${dr.height}`);
      cl.setAttribute("width",  String(canvas.width));
      cl.setAttribute("height", String(canvas.height));
      const svgBlob = new Blob([new XMLSerializer().serializeToString(cl)], { type: "image/svg+xml" });
      const svgUrl  = URL.createObjectURL(svgBlob);
      await new Promise<void>(r => {
        const ov = new Image();
        ov.onload  = () => { ctx.drawImage(ov, 0, 0, canvas.width, canvas.height); URL.revokeObjectURL(svgUrl); r(); };
        ov.onerror = () => { URL.revokeObjectURL(svgUrl); r(); };
        ov.src = svgUrl;
      });
    }
    ...
    ctx.fillRect(0, canvas.height - 50 * sc, 260 * sc, 50 * sc);
    ctx.fillStyle = "#e2e8f0";
    ctx.fillText(`Slice: ${panel.currentIndex + 1}/${panel.images.length}`, 8 * sc, canvas.height - 32 * sc);
    ctx.fillText(
      panel.pixelSpacing
        ? `${panel.pixelSpacing.x.toFixed(3)}×${panel.pixelSpacing.y.toFixed(3)} mm/px`
        : "sin calibración DICOM",
      8 * sc, canvas.height - 16 * sc,
    );
    ...
    canvas.toBlob(blob => {
      ...
      a.download = `tomografia_slice_${panel.currentIndex + 1}.png`;
      a.click();
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        setMeasurementMode(null); setActiveTool(null);
        setIsMeasureMenuOpen(false); setIsViewerMenuOpen(false);
        setIsLayoutMenuOpen(false); setIsViewMenuOpen(false);
      }
      if (e.key === "ArrowRight" || e.key === "ArrowDown")
        updatePanel(activePanel)(p => p.currentIndex < p.images.length - 1 ? { ...p, currentIndex: p.currentIndex + 1 } : p);
      if (e.key === "ArrowLeft" || e.key === "ArrowUp")
        updatePanel(activePanel)(p => p.currentIndex > 0 ? { ...p, currentIndex: p.currentIndex - 1 } : p);
    };
  ...
  return (
    <div className="bg-gray-900 text-white flex flex-col h-[calc(100vh-9rem)] max-h-[calc(100vh-9rem)] min-h-0 rounded-xl shadow-lg overflow-hidden" style={{ overscrollBehavior: "none" }}>
      ...
      <div className="bg-black px-5 border-b border-gray-700 shrink-0">
        <div className="flex justify-between items-center py-3">
          ...
          <div className="flex items-center gap-1 text-slate-300">
            ...
            {/* Contrast */}
            <button type="button" title="Brillo/Contraste — arrastra horizontal=contraste, vertical=brillo"
            ...
            <div className="relative" ref={viewerMenuRef}>
              <button type="button" title="Herramientas del visor"
                className={`h-9 w-9 rounded-md transition-colors flex items-center justify-center ${(activeP.cineActive || activeP.inverted || activeP.rotation !== 0 || activeP.zoom !== 1) ? "bg-indigo-500/20 text-indigo-300" : "hover:bg-white/10"}`}
              ...
              {isViewerMenuOpen && (
                <div className="absolute right-0 top-11 z-40 w-64 rounded-xl border border-slate-700 bg-[#0f172a] p-3 shadow-2xl">
                    ...
                    {activeP.cineActive && (
                      <div className="mt-2 flex items-center gap-2">
                        <div className="flex-1 bg-slate-700 rounded-full h-1 overflow-hidden">
                          <div className="h-full bg-indigo-400 transition-all duration-100" style={{ width: `${((activeP.currentIndex + 1) / Math.max(1, activeP.images.length)) * 100}%` }} />
                        </div>
                  ...
                  <div className="mb-2 rounded-lg border border-slate-700/60 bg-slate-800/40 p-2">
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => setPanels(prev => { const n = [...prev]; n[activePanel] = { ...n[activePanel], rotation: (n[activePanel].rotation - 90 + 360) % 360 }; return n; })}
                      ...
                      <button type="button" onClick={() => setPanels(prev => { const n = [...prev]; n[activePanel] = { ...n[activePanel], rotation: (n[activePanel].rotation + 90) % 360 }; return n; })}
                        className="flex-1 flex items-center justify-center gap-1 rounded-lg px-2 py-1.5 text-xs text-slate-200 bg-slate-700 hover:bg-slate-600 transition-colors">
                  ...
                  <button type="button"
                    ...
                    className={`w-full mb-2 flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition-colors ${activeP.inverted ? "bg-amber-500/15 text-amber-300 border border-amber-500/30" : "text-slate-200 bg-slate-800/40 border border-slate-700/60 hover:bg-slate-700/70"}`}>
                  ...
                  <div className="rounded-lg border border-slate-700/60 bg-slate-800/40 p-2">
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => setPanels(prev => { const n = [...prev]; n[activePanel] = { ...n[activePanel], zoom: Math.max(0.25, +(n[activePanel].zoom - 0.25).toFixed(2)) }; return n; })}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex justify-between text-[9px] text-slate-500 mt-0.5">
                          <span>25%</span>
                          <span className="text-slate-300 font-mono">{Math.round(activeP.zoom * 100)}%</span>
                          <span>400%</span>
                        </div>
                      </div>
                      <button type="button" onClick={() => setPanels(prev => { const n = [...prev]; n[activePanel] = { ...n[activePanel], zoom: Math.min(4, +(n[activePanel].zoom + 0.25).toFixed(2)) }; return n; })}
                        className="w-8 h-8 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-200 flex items-center justify-center transition-colors">
                        <ZoomIn className="h-4 w-4" strokeWidth={1.8} />
                      </button>
                    </div>
                    <button type="button" onClick={() => setPanels(prev => { const n = [...prev]; n[activePanel] = { ...n[activePanel], zoom: 1 }; return n; })}
        ...
        {/* Active tool pills */}
        {(measurementMode || activeTool || activeP.cineActive || activeP.inverted || activeP.rotation !== 0 || activeP.zoom !== 1) && (
          <div className="flex items-center gap-2 pb-2 flex-wrap">
            ...
            {activeTool === "contrast" && (
              <div className="flex items-center gap-2 rounded-full bg-purple-500/15 border border-purple-500/40 px-3 py-1">
                ...
                <button type="button" onClick={() => setPanels(prev => { const n = [...prev]; n[activePanel] = { ...n[activePanel], brightness: 256, contrast: 256 }; return n; })}
                  className="ml-1 text-purple-400 hover:text-purple-200 text-[10px] border border-purple-500/40 rounded px-1">Reset</button>
            ...
            {activeP.cineActive && (
              <div className="flex items-center gap-2 rounded-full bg-indigo-500/15 border border-indigo-500/40 px-3 py-1">
                ...
                <button type="button" onClick={() => setPanels(prev => { const n = [...prev]; n[activePanel] = { ...n[activePanel], cineActive: false }; return n; })}
                  className="ml-1 text-indigo-400 hover:text-indigo-200 text-xs leading-none">❚❚</button>
              </div>
            )}
            {activeP.inverted && (
              <div className="flex items-center gap-2 rounded-full bg-amber-500/15 border border-amber-500/40 px-3 py-1">
                ...
                <button type="button" onClick={() => setPanels(prev => { const n = [...prev]; n[activePanel] = { ...n[activePanel], inverted: false }; return n; })}
                  className="ml-1 text-amber-400 hover:text-amber-200 text-xs leading-none">✕</button>
              </div>
            )}
      ...
      <div className="flex-1 min-h-0 flex bg-black overflow-hidden">
            ...
            {patientStudies.length > 0 ? patientStudies.map(study => {
              ...
              return (
                <div
                  ...
                  onClick={() => fetchForPanel(activePanel, study.orthancStudyId, activeView)}
        ...
        {/* Panel grid */}
        <div className={`flex-1 min-w-0 min-h-0 grid ${gridCls} gap-px bg-gray-800 relative`}>
          <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200&icon_names=text_rotation_none" />

          {/* View selector */}
          <div className="absolute top-3 left-3 z-40" ref={viewMenuRef}>
            <button type="button" onClick={() => setIsViewMenuOpen(v => !v)}
              className="h-9 w-9 rounded-md bg-slate-900/80 border border-slate-700 hover:bg-slate-800/90 transition-colors flex items-center justify-center"
              title="Vista">
              <span className="material-symbols-outlined text-slate-200 text-[20px]">text_rotation_none</span>
            </button>
            {isViewMenuOpen && (
              <div className="mt-2 w-40 rounded-lg border border-slate-700 bg-[#0f172a] shadow-2xl p-1">
                {(["sagital", "coronal", "axial"] as const).map(view => (
                  <button key={view} type="button"
                    onClick={() => handleSelectView(view)}
                    className={`w-full text-left px-3 py-2 text-sm rounded-md transition-colors capitalize ${activeView === view ? "bg-indigo-500/20 text-indigo-200" : "text-slate-200 hover:bg-slate-700/70"}`}>
                    {view}
                  </button>
                ))}
              </div>
